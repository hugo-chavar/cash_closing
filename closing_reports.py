...
def extract_cash_closing_totals(cash_closing):
    ...
    if state != "DELETED":
        ...
        cash_amount = next(
            (
                pt["amount"]
                for pt in cash_closing["cash_statement"]["payment"]["payment_types"]
                if pt["type"] == "Bar"
            ),
            0,
        )
        non_cash_amount = next(
            (
                pt["amount"]
                for pt in cash_closing["cash_statement"]["payment"]["payment_types"]
                if pt["type"] == "Unbar"
        ...
        vat_data_1 = next(
            (
                item
                for item in cash_closing["cash_statement"]["business_cases"][0][
                    "amounts_per_vat_id"
                ]
                if item["vat_definition_export_id"] == 1
            ),
            {},
        )
        vat_data_2 = next(
            (
                ...
                if item["vat_definition_export_id"] == 2
            ),
            {},
        )
        ...
        for tx in cash_closing["transactions"]:
            ...
            if payment["type"] == "Bar":
                ...
                if tx_payment_amount != cash_tx_total:
                    print(
                        f"Tx problem {tx['head']['number']} payment_amout {tx_payment_amount} cash total {cash_tx_total}"
                    )
            else:
                non_cash_tx_total = float(non_cash_tx["vat_19"] + non_cash_tx["vat_7"])
                if tx_payment_amount != non_cash_tx_total:
                    print(
                        f"Tx problem {tx['head']['number']} payment_amout {tx_payment_amount} cash total {non_cash_tx_total}"
                    )
        ...
        with open(os.path.join(folder_path, file_name), "r", encoding="utf-8") as file:
            ...
            if id >= 379:
                (
                    ...
                    cash_totals,
                    non_cash_totals,
                ) = extract_cash_closing_totals(cash_closing)
                data.append(
                    [
                        id,
                        full_amount,
                        ...
                        cash_totals["vat_7"],
                        non_cash_amount,
                        non_cash_totals["vat_19"],
                        non_cash_totals["vat_7"],
                        time_creation,
                    ]
                )
