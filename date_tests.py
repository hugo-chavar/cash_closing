...
def get_truncated_date(original_date_time):
    truncated_date_time = original_date_time.replace(
...
def print_german_date(number, original_timestamp):
    ...
    original_date_time_utc = datetime.fromtimestamp(
        original_timestamp, tz=datetime.timezone.utc
    )
    ...
    # Set time to midnight
    truncated_date_time_germany = original_date_time_germany.replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    ...
    print(
        f"{number}: {original_timestamp} - {original_date_time_germany.strftime("%d/%m/%Y, %H:%M:%S")} - {int(truncated_date_time_germany.timestamp())} {truncated_date_time_germany.strftime("%d/%m/%Y, %H:%M:%S")} "
    )
...
def get_yesterday_end_timestamp():
    ...
    # Subtract 1 second to get yesterday at 23:59:59
    yesterday_end = today - timedelta(seconds=1)
    # Convert to timestamp
    return int(yesterday_end.timestamp())
